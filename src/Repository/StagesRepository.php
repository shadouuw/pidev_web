...
public function Total_Stages()
{
    ...
    function totaltemps($id)
    {

        $qb=$this->createQueryBuilder('s')->select('sum(s.temps)')
            ->where('s.jeu=:isSubscribe')
            ->setParameter('isSubscribe', $id)
            ->setMaxResults(1)
            ->getQuery()
    ...
    function max($id)
    {
        ...
        $em = $this->getEntityManager();
        $query = $em->createQuery(
            'SELECT max(s.score) FROM App\Entity\Scores s where s.idJeux=:isSubscribe'
        );
        $query->setParameter('isSubscribe', $id);
